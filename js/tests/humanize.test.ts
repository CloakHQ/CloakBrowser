...
// =========================================================================
// humanType non-ASCII support
// =========================================================================
describe("humanType non-ASCII", () => {
  it("handles Cyrillic text without throwing", async () => {
    ...
    const raw = {
      down: vi.fn(async (key: string) => { downKeys.push(key); }),
      up: vi.fn(async () => {}),
      type: vi.fn(async () => {}),
      insertText: vi.fn(async (text: string) => { insertedChars.push(text); }),
    };

    const page = buildMockPage();

    await humanType(page, raw, "Привет", cfg);

    expect(insertedChars.join("")).toBe("Привет");
    for (const key of downKeys) {
      expect(key.charCodeAt(0)).toBeLessThan(128);
    }
  });

  it("handles mixed ASCII and non-ASCII text", async () => {
    const { humanType } = await import("../src/human/keyboard.js");
    const cfg = resolveConfig("default");
    ...
    await humanType(page, raw, "Hello Мир!", cfg);

    expect(insertedChars).toContain("М");
    expect(insertedChars).toContain("и");
    expect(insertedChars).toContain("р");

    expect(downKeys).toContain("e");
    expect(downKeys).toContain("l");
    expect(downKeys).toContain("o");
  });
  ...
  it("handles CJK characters", async () => {
    ...
    await humanType(page, raw, "你好世界", cfg);

    expect(insertedChars.join("")).toBe("你好世界");
  });
  ...
  it("handles emoji", async () => {
    ...
    await humanType(page, raw, "Hi 👋", cfg);

    expect(insertedChars.length).toBeGreaterThan(0);
  });

  it("mistype only triggers for ASCII alphanumeric", async () => {
    const { humanType } = await import("../src/human/keyboard.js");
    ...
    await humanType(page, raw, "Я", cfg);

    expect(downKeys).not.toContain("Backspace");
  });

  it("mistype triggers Backspace for ASCII", async () => {
    const { humanType } = await import("../src/human/keyboard.js");
    const cfg = resolveConfig("default", { mistype_chance: 1.0 });
    ...
    await humanType(page, raw, "a", cfg);
