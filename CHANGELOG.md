...
- **[wrapper]** **The WebRTC exit IP now covers both address families on a dual-stack proxy.** The egress lookup only ever queried endpoints that publish an IPv4 record, so a proxy whose exit reaches IPv6 destinations over IPv6 still reported its IPv4 address, and that address alone was used for the WebRTC exit IP. A second lookup now resolves the IPv6 exit as well and both are passed when the proxy has them; a proxy without IPv6 is unchanged and pays only a short failed lookup. Version-gated: binaries that accept a single address only keep receiving one. Reported by Albert Tang (Nylas). Python, JavaScript, and .NET.
