...
class TestNonAsciiKeyboard:
    """Tests for Cyrillic, CJK, emoji support in humanType."""
    ...
    def test_cyrillic_uses_insert_text(self):
        ...
        # All Cyrillic chars should go through insert_text
        assert "".join(inserted) == "Привет"
        # No Cyrillic char should go through down()
        for key in down_keys:
            assert ord(key[0]) < 128 or key in ("Shift", "Backspace")
    ...
    def test_mixed_ascii_and_cyrillic(self):
        ...
        human_type(page, raw, "Hello Мир", cfg)

        # Non-ASCII via insert_text
        assert "М" in inserted
        assert "и" in inserted
        assert "р" in inserted

        # ASCII via down/up
        assert "e" in down_keys
        assert "l" in down_keys
    ...
    def test_cjk_characters(self):
        ...
        human_type(page, raw, "你好世界", cfg)

        assert "".join(inserted) == "你好世界"
    ...
    def test_mistype_only_ascii(self):
        ...
        cfg = resolve_config("default", {"mistype_chance": 1.0})  # 100%
        down_keys = []
        ...
        human_type(page, raw, "Я", cfg)

        # No Backspace for non-ASCII
        assert "Backspace" not in down_keys

    def test_mistype_triggers_for_ascii(self):
        from cloakbrowser.human.keyboard import human_type
        from cloakbrowser.human.config import resolve_config
        from unittest.mock import MagicMock
        ...
        human_type(page, raw, "a", cfg)

        assert "Backspace" in down_keys

    def test_no_error_on_cyrillic(self):
        """Regression test: should not raise 'Unknown key' error."""
        from cloakbrowser.human.keyboard import human_type
        from cloakbrowser.human.config import resolve_config
        from unittest.mock import MagicMock
...
class TestNonAsciiKeyboardAsync:
    """Async version of non-ASCII tests."""

    def test_async_cyrillic(self):
        ...
        raw = AsyncMock()
        raw.down = AsyncMock(side_effect=lambda k: down_keys.append(k))
        raw.up = AsyncMock()
        ...
        asyncio.get_event_loop().run_until_complete(
            async_human_type(page, raw, "Привет", cfg)
        )
    ...
    def test_async_mixed(self):
        ...
        asyncio.get_event_loop().run_until_complete(
            async_human_type(page, raw, "Hi Мир", cfg)
        )
...
class TestBrowserBotDetection:
    PROXY = None  # Set to valid proxy URL or None to disable

    def test_behavioral_checks_pass(self):
        from cloakbrowser import launch
        launch_kwargs = {"headless": False, "humanize": True}
        if self.PROXY:
