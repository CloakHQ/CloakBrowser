...
async def async_human_type(page: Any, raw: AsyncRawKeyboard, text: str, cfg: HumanConfig) -> None:
    for i, ch in enumerate(text):
        # Mistype only for ASCII alphanumeric (NEARBY_KEYS is Latin-only)
        if random.random() < cfg.mistype_chance and ch.isalnum() and ch.isascii():
            wrong = _get_nearby_key(ch)
            await _async_type_char(page, raw, wrong, cfg)
        ...
        await _async_type_char(page, raw, ch, cfg)

        if i < len(text) - 1:
            await _inter_char_delay(cfg)


async def _async_type_char(page: Any, raw: AsyncRawKeyboard, ch: str, cfg: HumanConfig) -> None:
    """Route character input: ASCII via down/up, non-ASCII via insert_text."""
    if not ch.isascii():
        await async_sleep_ms(rand_range(cfg.key_hold))
